...
import { AlertCircle, LogOut, UserPlus, Key, ShieldCheck, ShieldAlert } from "lucide-react"
...
export default function SecureDashboardPage() {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        ...
        // Fetch customers using secure endpoint
        fetchCustomers()
      } catch (err) {
        console.error(err)
        router.push("/secure/login")
  ...
  const fetchCustomers = async () => {
    try {
      // Use secure endpoint
      const response = await fetch("/api/customers/secure")
      if (response.ok) {
        const data = await response.json()
  ...
  const handleAddCustomer = async (e: React.FormEvent) => {
    ...
    try {
      // Use secure endpoint with parameterized queries
      const response = await fetch("/api/customers/secure", {
        method: "POST",
        headers: {
        ...
        <div className="container mx-auto px-4 flex justify-between items-center">
          <div className="flex items-center">
            ...
            <div className="ml-4 flex items-center">
              ...
              <span className="text-sm font-medium text-green-500">Secure Mode</span>
            </div>
          </div>
          {user && (
            <div className="flex items-center gap-4">
              ...
              <Button variant="secondary" size="sm" asChild>
                <Link href="/secure/change-password">
